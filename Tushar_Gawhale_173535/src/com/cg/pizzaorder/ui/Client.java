...
public class Client {
	public static void main(String[] args) {
		...
		PizzaOrder pizza = new PizzaOrder();   //changed null to pizza order
		...
		for (;;) {
			...
			if (menuChoice.equals("1"))// place order
			{
				...
				while (true)// set address
				{
					...
					try {
						boolean isValid = service.validateAddress(address);  //changed 
						if (isValid) {
			...
			else if (menuChoice.equals("2"))// start of second choice
			{
				try {
					...
					//get data of order
					PizzaOrder getPizza = service.getOrderDetails(Integer.parseInt(orderID));  // changed from 0 to orderId
